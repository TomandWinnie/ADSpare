...
/*
 * 原⽣广告视图模板广告view点击关闭按钮时调用，作用于 AdCompNativeAdType_View(视图广告) 广告下
...
typedef NS_ENUM(NSUInteger, AdCompNativeAdType) {
    AdCompNativeAdType_Data = 0,  // 物料类型
...
/*
 *原生广告视图注册方法
 *@param nativeData 数据对象，物料类型必传字段
 *@param clickableView 可点击的视图，响应广告的点击事件
 */
- (void)registerNativeAdWithData:(AdCompNativeData *)nativeData
                   clickableView:(UIView *)clickableView;
